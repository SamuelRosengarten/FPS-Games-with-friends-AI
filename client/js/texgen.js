// Procedural PBR texture generation (albedo, normal, roughness/metalness/height) as plain typed arrays.
// No three.js or DOM here, so it runs both on the main thread and inside texture workers (texworker.js).
...
function toTextureData(tc, normalStrength) {
  const S = tc.size;
  ...
  return { size: S, albedo, normal, rough, emissive };
}
...
// Pixel size a material is generated at: big surfaces at the full library size, the rest capped.
export function textureSizeFor(name, size, smallSize) {
  const d = MATERIAL_DEFS[name] || MATERIAL_DEFS.concrete;
  return d.scale >= 4 || d.r === 'corrugated' ? size : Math.min(size, smallSize);
}

// Generate one material's texture data. Returns { size, albedo, normal, rough, emissive|null } (RGBA8).
export function generateTextureData(name, size) {
  const d = MATERIAL_DEFS[name] || MATERIAL_DEFS.concrete;
  const tc = new TexCanvas(size);
  const seed = [...name].reduce((a, c) => a * 31 + c.charCodeAt(0), 7) >>> 0;
  const n = new Noise(seed);
  RECIPES[d.r](tc, n, d.opt || {});
  return toTextureData(tc, d.ns * (size / 512));
}
