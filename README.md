...
from the lobby. Players can join a match that is already running.
...
- `server/` — zero-dependency Node server: HTTP + WebSocket (`websocket.js`), lobby & host controls (`game.js`),
  match simulation with rounds/economy/grenades/bomb/lag compensation (`match.js`), bot AI (`bot.js`) and navigation mesh (`nav.js`)
